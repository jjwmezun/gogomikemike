...
class WPMetaBox
{
	...
	/////////////////////////////////////////////////////////
		...
		public function __construct( string $slug, string $title, array $extra_attributes = [] )
		{
			$this->slug = $slug;
			$this->title = $title;
            $this->domain = $extra_attributes[ 'domain' ] ?? 'gogomikemike';
            $this->priority = $extra_attributes[ 'priority' ] ?? 'default';
            $this->context = $extra_attributes[ 'context' ] ?? 'side';

            if ( !empty( $extra_attributes[ 'post-type' ] ) )
            {
                // If singular, make it an array with singlular as only child.
                $this->postTypes = ( !is_array( $extra_attributes[ 'post-type' ] ) )
                    ? [ $extra_attributes[ 'post-type' ] ]
                    : $extra_attributes[ 'post-type' ];
            }
	...
	/////////////////////////////////////////////////////////
		...
		{
			return isset( $_POST[ 'post_type' ] ) && in_array( $_POST[ 'post_type' ], $this->postTypes );
		}
